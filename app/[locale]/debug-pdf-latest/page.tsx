...
export default function DebugPdfLatestPage() {
  ...
  useEffect(() => {
    addLog('🚀 全新PDF调试页面启动 - 版本 v3.0 [2025-07-07-04:05] - 绕过缓存')
    ...
    addLog('🔍 URL查询字符串: ' + window.location.search)
    addLog('🎯 提取的PDF ID: ' + (pdfId || 'null'))
    
    if (pdfId) {
      addLog('✅ 检测到PDF ID，将执行特定PDF测试')
    } else {
      addLog('ℹ️ 未提供PDF ID，将执行基础功能测试')
    }
    ...
    const testPDFFunction = async () => {
      ...
      const urlParams = new URLSearchParams(window.location.search)
      addLog('🔍 重新解析URL参数: ' + window.location.search)
      
      const pdfId = urlParams.get('id')
      addLog('🎯 重新提取PDF ID: ' + (pdfId || 'null'))
      addLog('🔬 PDF ID类型检查: ' + typeof pdfId)
      addLog('🔬 PDF ID长度: ' + (pdfId ? pdfId.length : 0))
      
      if (pdfId && pdfId.trim() !== '') {
        addLog('✅ 条件判断: 有有效PDF ID，执行特定PDF测试')
        await testSpecificPDF(pdfId.trim())
      } else {
        addLog('✅ 条件判断: 无有效PDF ID，执行基础测试')
    ...
    const testSpecificPDF = async (pdfId: string) => {
      try {
        ...
        addLog(`📋 完整PDF信息: ${JSON.stringify(pdfInfo, null, 2)}`)
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow p-6">
          <h1 className="text-2xl font-bold mb-4 text-red-600">🔍 最新PDF错误诊断工具</h1>
          <p className="text-gray-600 mb-6">
            全新版本，绕过缓存问题。专门用于诊断PDF相关的client-side exception错误。
