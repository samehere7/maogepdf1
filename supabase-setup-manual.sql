...
-- 允许认证用户调用get_user_plus_status查询自己的状态
...
COMMENT ON FUNCTION update_user_plus_status(UUID, BOOLEAN, BOOLEAN, TIMESTAMPTZ, TEXT) IS 'Updates user Plus status from Paddle webhook';
COMMENT ON FUNCTION get_user_plus_status(UUID) IS 'Gets user Plus status with expiration check';
...
BEGIN
  ...
  IF function_count = 2 AND view_exists THEN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🚀 支付系统数据库配置完成，可以部署应用！';
  ELSE
